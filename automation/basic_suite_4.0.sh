...
cleanup() {
    ...
    [[ -d deployment-basic_suite_4.0/current/logs ]] \
    && mv deployment-basic_suite_4.0/current/logs exported-artifacts/lago_logs
    find deployment-basic_suite_4.0 \
        -iname nose\*.xml \
        -exec mv {} exported-artifacts/ \;
    [[ -d test_logs ]] && mv test_logs exported-artifacts/
    ./run_suite.sh --cleanup basic_suite_4.0
    exit
}
