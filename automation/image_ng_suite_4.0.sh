...
cleanup() {
    ...
    [[ -d deployment-image_ng_suite_4.0/current/logs ]] \
    && mv deployment-image_ng_suite_4.0/current/logs exported-artifacts/lago_logs
    find deployment-image_ng_suite_4.0 \
        -iname nose\*.xml \
        -exec mv {} exported-artifacts/ \;
    [[ -d test_logs ]] && mv test_logs exported-artifacts/
    ./run_suite.sh --cleanup image_ng_suite_4.0
    exit
}
...
./run_suite.sh image_ng_suite_4.0 \
    -n "$node" \
    -e "$engine" \
|| res=$?
exit $res
