...
VLAN100_NET_IPv4_ADDR = '192.0.2.{}'
VLAN100_NET_IPv4_MASK = '255.255.255.0'
VLAN100_NET_IPv6_ADDR = '2001:0db8:85a3:0000:0000:8a2e:0370:733{}'
...
def prepare_migration_vlan(api):
    usages = params.Usages(['vm', 'migration'])
...
def migrate_vm(prefix, api):
    def current_running_host():
        host_id = api.vms.get(VM0_NAME).get_host().get_id()
        return api.hosts.get(id=host_id).get_name()
...
@testlib.with_ovirt_api
def prepare_mig_attachments_ipv6(api):
    for index, host in enumerate(_hosts_in_cluster(api, CLUSTER_NAME),
                                 start=1):
        ip_address = VLAN100_NET_IPv6_ADDR.format(index)

        ip_configuration = network_utils.create_static_ip_configuration(
            ipv6_addr=ip_address,
            ipv6_mask=VLAN100_NET_IPv6_MASK)
