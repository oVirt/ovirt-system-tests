...
def verify_migration_flags(ansible_by_hostname, src_host, vm_id, dst_host_name, expected_flags, forbidden_flags=0):
    """Verify vdsm.log shows the migration was started with expected flags.

    Flags in forbidden_flags must not be set.
    """
    ...
    assert flags & forbidden_flags == 0, (
        f'Forbidden flags {forbidden_flags} (compressed={VIR_MIGRATE_COMPRESSED}, '
        f'parallel={VIR_MIGRATE_PARALLEL}) set in {flags} from line: {line}'
    )
...
def test_parallel_migration(
    ...
    try:
        ...
        verify_migration_flags(
            ...
            forbidden_flags=VIR_MIGRATE_COMPRESSED,
        )
    finally:
