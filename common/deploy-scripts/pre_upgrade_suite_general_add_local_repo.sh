...
cd  /etc/yum.repos.d/
yum install --disablerepo=\* --enablerepo="$(cat pre-reposync-config*.repo | grep '\['| tr -d '[]'| grep -v main | xargs | tr ' ' ',')" -y yum-utils
yum-config-manager --disable \*

for i in $(cat pre-reposync-config*.repo | grep '\['| tr -d '[]'| grep -v main); do
