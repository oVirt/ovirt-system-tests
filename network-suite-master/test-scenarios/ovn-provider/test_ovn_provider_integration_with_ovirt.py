...
def running_vm_0(
    ...
    with virtlib.vm_pool(system, size=1) as (vm_0,):
        vm_0.create(
            vm_name=VM0_NAME, cluster=default_cluster, template=cirros_template
        )
        ...
        vm0_vnic_0.create(
            name=VNIC0_NAME, vnic_profile=vnic_profile0, mac_addr=VNIC0_MAC
        )
...
def test_connect_vm_to_external_network(
    running_vm_0, default_ovn_provider_client
):
...
def test_modify_vnic_sec_groups_on_ext_networks(
    ...
    with netlib.create_vnic_profile(
        system, 'temporary', ovirt_external_network
    ) as profile:
        with _create_security_group(
            default_ovn_provider_client, 'temporary', 'temporary sec group'
        ) as sec_group:
