...
def test_bond_active_slave(system, default_data_center, default_cluster, host_0_up):
    ...
    with hostlib.setup_networks(host_0_up, bonding_data=(bond_data,)):
        ...
        sshlib.Node(host_0_up.address, host_0_up.root_password).change_active_slave(BOND_NAME, inactive_slave.name)
        try:
            syncutil.sync(
                exec_func=lambda: bond.active_slave,
                exec_func_args=(),
                success_criteria=lambda active_slave: active_slave.id != initial_active_slave.id,
                timeout=10,
            )
