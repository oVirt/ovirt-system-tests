...
MAC_POOL_RANGE = clusterlib.MacPoolRange(start=MAC_ADDR_1, end=MAC_ADDR_2)
MAC_POOL_RANGE_1 = clusterlib.MacPoolRange(start=MAC_ADDR_3, end=MAC_ADDR_4)
MAC_POOL_RANGE_3 = clusterlib.MacPoolRange(start=MAC_ADDR_5, end=MAC_ADDR_6)
...
def test_undo_preview_snapshot_when_mac_used_reassigns_a_new_mac(
    ...
    with virtlib.vm_pool(system, size=2) as (vm_0, vm_1):
        vm_0.create(
            vm_name='test_undo_preview_snapshot_when_mac_used_vm_0',
            cluster=default_cluster,
        ...
        vm_1.create(
            vm_name='test_undo_preview_snapshot_when_mac_used_vm_1',
            cluster=default_cluster,
            template=cirros_template,
        )
        vm_1.create_vnic(NIC_NAME_1, ovirtmgmt_vnic_profile, MAC_ADDR_1)
...
def test_mac_pools_in_different_clusters_dont_overlap(system, cluster_0, default_cluster, ovirtmgmt_vnic_profile):
...
def test_add_overlapping_mac_pool_same_cluster(system, cluster_0, default_cluster):
    POOL_0 = 'mac_pool_0'
    POOL_1 = 'mac_pool_1'
...
@suite.skip_suites_below('4.4')
def test_add_overlapping_mac_pool_other_cluster(system, cluster_0, default_cluster):
    POOL_0 = 'mac_pool_0'
    POOL_1 = 'mac_pool_1'
    default_cluster_mac_pool = clusterlib.mac_pool(system, default_cluster, POOL_0, (MAC_POOL_RANGE,))
...
def test_restore_snapshot_with_an_used_mac_implicitly_assigns_new_mac(
    ...
    with virtlib.vm_pool(system, size=2) as (vm_0, vm_1):
        ...
        vnic_0 = vm_0.create_vnic(NIC_NAME_1, ovirtmgmt_vnic_profile, MAC_ADDR_1)
        vm_0.wait_for_down_status()
...
def test_move_stateless_vm_mac_to_new_vm_fails(
    ...
    with virtlib.vm_pool(system, size=2) as (vm_0, vm_1):
        vm_0.create(
            vm_name='test_move_stateless_vm_mac_to_new_vm_fails_vm_0',
            cluster=default_cluster,
            template=cirros_template,
            stateless=True,
        )
        ...
        vm_1.create(
            vm_name='test_move_stateless_vm_mac_to_new_vm_fails_vm_1',
            cluster=default_cluster,
            template=cirros_template,
        )
...
def test_move_mac_to_new_vm(
    ...
    with virtlib.vm_pool(system, size=2) as (vm_0, vm_1):
        vm_0.create(
            vm_name='test_move_mac_to_new_vm_0',
            cluster=default_cluster,
            template=cirros_template,
        )
        ...
        vm_1.create(
            vm_name='test_move_mac_to_new_vm_1',
            cluster=default_cluster,
            template=cirros_template,
        )
...
def test_allocate_mac_in_use_in_other_cluster_small_mac_pool(system, default_data_center, cluster_0, cluster_1):
    """
    A 2-address pool is exhausted by the flow and the pool is tested for being
    able to find another free and unused address.
    """
    _run_scenario_of_bz_1760170(system, default_data_center, cluster_0, cluster_1)


@suite.skip_suites_below('4.4')
@pytest.mark.usefixtures('pool_3_cluster_0', 'pool_1_cluster_1')
@pytest.mark.usefixtures('host_0_in_cluster_0', 'host_1_in_cluster_1')
def test_allocate_mac_in_use_in_other_cluster_large_mac_pool(system, default_data_center, cluster_0, cluster_1):
    """
...
def pool_1_cluster_1(system, cluster_1):
    with clusterlib.mac_pool(system, cluster_1, 'p1', (MAC_POOL_RANGE_1,)) as p1:
        yield p1


@pytest.fixture(scope='function')
def pool_3_cluster_0(system, cluster_0):
    with clusterlib.mac_pool(system, cluster_0, 'p3', (MAC_POOL_RANGE_3,)) as p3:
        yield p3
