...
def test_add_overlapping_mac_pool_same_cluster(
    system, cluster_0, default_cluster
):
    POOL_0 = 'mac_pool_0'
    POOL_1 = 'mac_pool_1'
    default_cluster_mac_pool = clusterlib.mac_pool(
...
@suite.skip_suites_below('4.4')
def test_add_overlapping_mac_pool_other_cluster(
    system, cluster_0, default_cluster
):
    POOL_0 = 'mac_pool_0'
    POOL_1 = 'mac_pool_1'
...
def _run_scenario_of_bz_1760170(system, default_dc, cluster_0, cluster_1):
    ...
    with clusterlib.new_assigned_network(
        ...
        with virtlib.vm_pool(system, size=2) as (vm_0, vm_1):
            ...
            vm_0.wait_for_down_status()
            vm_0.create_vnic(
                netlib.OVIRTMGMT, default_dc.get_mgmt_network().vnic_profile()
            )
...
def pool_1_cluster_1(system, cluster_1):
    with clusterlib.mac_pool(
        system, cluster_1, 'p1', (MAC_POOL_RANGE_1,)
    ) as p1:
        yield p1
...
def pool_3_cluster_0(system, cluster_0):
    with clusterlib.mac_pool(
        system, cluster_0, 'p3', (MAC_POOL_RANGE_3,)
    ) as p3:
        yield p3
