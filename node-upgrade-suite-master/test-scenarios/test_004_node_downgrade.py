...
cmd_for_getting_unactive_layer = "imgbase layout | grep ^[a-z] |" " grep -v $(imgbase w | grep -o 'ovirt[^+]*')"
...
def _remove_layer(ansible_host):
    base_to_remove = ansible_host.shell(cmd_for_getting_unactive_layer)['stdout']
    ansible_host.shell(cmd_for_removing_layer.format(base_to_remove))


def _revert_yum_status(ansible_host):
    cur_ver = ansible_host.shell(cmd_for_getting_current_version)['stdout']
    cmd = 'curl ' + base_url + f' | grep -Po "{ver_package}-{cur_ver}.[^ \'<]*"| tail -n1'
    ...
    ansible_host.shell("yum remove -y ovirt-node-ng-image-update-placeholder")
