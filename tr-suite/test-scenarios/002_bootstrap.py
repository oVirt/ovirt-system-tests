...
@order_by(_TEST_LIST)
def test_sync_time(ansible_host0, engine_hostname):
    ansible_host0.shell('chronyc -4 add server {}'.format(engine_hostname))
    ansible_host0.shell('chronyc -4 makestep')


@order_by(_TEST_LIST)
def test_add_hosts(ansible_host0_facts, engine_api, root_password):
    engine = engine_api.system_service()

    hostnames = [
        facts.get("ansible_hostname")
        for facts in [ansible_host0_facts]
    ]
...
@order_by(_TEST_LIST)
def test_terraform(engine_full_username, engine_api_url, engine_api):
